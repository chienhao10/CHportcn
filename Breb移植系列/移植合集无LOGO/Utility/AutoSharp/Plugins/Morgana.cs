            ...
            try
            {
                if (ComboMode)
                {
                    ...
                    if (W.IsReady() && Heroes.Player.LSDistance(Target) < W.Range)
                    {
                                ...
                                .Where(
                                    hero =>
                                        (hero.HasBuffOfType(BuffType.Snare) || hero.HasBuffOfType(BuffType.Stun) ||
                                         hero.HasBuffOfType(BuffType.Taunt) && hero.LSIsValidTarget(W.Range))))
                        {
                        ...
                        foreach (var enemy in
                            ObjectManager.Get<AIHeroClient>().Where(hero => hero.LSIsValidTarget(W.Range)))
                        {
                    ...
                    if (R.IsReady() && Heroes.Player.LSDistance(Target) < R.Range &&
                        Helpers.EnemyInRange(ComboConfig["ComboCountR"].Cast<Slider>().CurrentValue, R.Range))
                    {
                        R.Cast();
                    }
                }
