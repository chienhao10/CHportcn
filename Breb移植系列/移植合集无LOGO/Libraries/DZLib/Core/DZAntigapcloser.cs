...
namespace DZLib.Core
{
    ...
    public static class DZAntigapcloser
    {
        ...
        {
            var GPMenu = mainMenu.AddSubMenu(title, name);
            {
                ...
                {
                    GPMenu.Add($"{name}.{gp.ChampionName.ToLowerInvariant()}.{gp.SpellName}", new CheckBox(gp.ChampionName + " " + gp.Slot + " (" + gp.SpellName + ")"));
                }
            }
            GapMenu = GPMenu;
            GPMenuName = name;
        ...
        private static void Game_OnGameUpdate(EventArgs args)
        {
            ...
            foreach (var gapcloser in ActiveGapclosers.Where(gapcloser => gapcloser.Sender.LSIsValidTarget()).Where(gapcloser => gapcloser.SkillType == GapcloserType.Targeted || (gapcloser.SkillType == GapcloserType.Skillshot && ObjectManager.Player.LSDistance(gapcloser.Sender, true) < 250000))) // 500 * 500
            {
                if (GapMenu[$"{GPMenuName}.{gapcloser.Sender.ChampionName.ToLowerInvariant()}.{gapcloser.SpellName}"] == null)
                {
                    return;
                }
