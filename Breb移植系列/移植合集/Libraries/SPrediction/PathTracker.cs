        ...
        private static void AIHeroClient_OnNewPath(Obj_AI_Base sender, GameObjectNewPathEventArgs args)
        {
            ...
            else
            {
                ...
                if (!enemy.LastWaypoints.SequenceEqual(wp))
                {
                    if (!enemy.IsStopped)
                    {
                        enemy.AvgTick = (enemy.Count*enemy.AvgTick + (Environment.TickCount - enemy.LastWaypointTick))/
                                        ++enemy.Count;
                    ...
                    if (!enemy.IsWindupChecked)
                    {
                        if (Environment.TickCount - enemy.LastAATick < 300)
                            enemy.AvgOrbwalkTime = (enemy.AvgOrbwalkTime*enemy.OrbwalkCount +
                                                    (enemy.LastWaypointTick - enemy.LastWindupTick))/
                                                   ++enemy.OrbwalkCount;
